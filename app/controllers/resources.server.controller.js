...
exports.update = function(req, res) {
	...
	resource = _.extend(resource , req.body);
		//depopulate
	if (resource.training.name) resource.training = resource.training._id;
...
exports.list = function(req, res) { 
	Resource.find().sort('-created').populate('training', 'name').exec(function(err, resources) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.resourceByID = function(req, res, next, id) { 
	Resource.findById(id).populate('training', 'name').exec(function(err, resource) {
